...
\begin{table}[htbp]
    ...
    \begin{threeparttable}
        ...
        \begin{tabular}{l c c c c}
            ...
            \midrule
            \rowcolor{gray!15} \textbf{本文 (Ours)} & \textbf{岸电+换电} & \textbf{并行 (SIMOPS)} & \textbf{列生成（CG）} & \textbf{高 (N=100, <20s)} \\
            \bottomrule
        \end{tabular}%
        }
        \begin{tablenotes}
...
表 \ref{tab:exp_params} 给出基准参数设置，表 \ref{tab:ship_types} 给出船舶类型结构。类型 C 不兼容岸电，其余类型可兼容岸电；褐色模式为保底选项。
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l c c c c c}
        \toprule
        类型 & 占比 & 能量需求 (kWh) & 延误惩罚 (\$/h) & 岸电兼容 & 装卸时长 (h) \\
        \midrule
        A & 0.20 & [6000, 8000] & 2000 & 是 & [6, 24] \\
        B & 0.50 & [3000, 5000] & 800 & 是 & [6, 24] \\
        C & 0.15 & [3000, 4000] & 500 & 否 & [6, 24] \\
        D & 0.15 & [4000, 4000] & 1500 & 是 & [6, 12] \\
        \bottomrule
    \end{tabular}
\end{table}
...
(ii) 相比 FIFO/Greedy，CG 在所有规模下均取得更低成本。以 $N=100$ 为例，CG 相比 Greedy/FIFO 分别降低约 12.5\%/13.1\%；在 $N=500$ 规模下仍可保持约 6\% 的降本。
(iii) CG 具有良好的可扩展性：平均求解时间从 $N=20$ 的 0.38s 增至 $N=500$ 的 16.2s，仍显著低于 MILP 在中等规模下的运行时间。
...
\paragraph{场景对比分析}
表 \ref{tab:scenario_results} 与图 \ref{fig:scenario_results} 展示不同到达/服务场景下的表现。峰值到达场景（P）因资源拥堵导致系统成本最高（CG 均值约 230.3k），而长作业场景（L）由于掩盖效应更充分，整体成本反而最低（约 217.3k）。在三类场景中 CG 均保持最优解质量，运行时间在 L 场景略升高（均值约 11.7s），但仍维持可接受的计算开销。
...
为量化 SIMOPS 的价值，设置 SIMOPS 与“串行作业”两种操作模式并进行对照实验。表 \ref{tab:simops_results} 与图 \ref{fig:simops_savings} 显示：SIMOPS 在所有规模下均可显著降低系统成本。以 CG 为例，平均降本约 9.1\%，在 $N=100$ 时降本约 14.1\%，呈现随规模增加而收益边际递减的趋势。
...
\begin{table}[htbp]
    ...
    \footnotesize
    \input{results/simops_table.tex}
\end{table}
...
图 \ref{fig:masking_dist} 给出了 SIMOPS 掩盖率分布，平均掩盖率约为 0.42，范围覆盖 $[0.07,0.92]$，说明该效应在不同实例中普遍存在但强度不同。按船舶类型拆分（图 \ref{fig:masking_type}），高时敏类型 A 的掩盖收益最大；不兼容岸电的类型 C 受益最小，体现了资源兼容性与掩盖效应的耦合关系。图 \ref{fig:gantt_simops} 进一步从单实例角度展示了 SIMOPS 下能源服务与装卸作业的时间重叠，从而减少实际离港延误。
...
图 \ref{fig:sens_battery}--\ref{fig:sens_deadline} 总结了关键参数的敏感性结果（$N=100$，U 场景）。电池成本由 0.30 增至 0.80 \$/kWh 时，系统总成本均值由约 175k 上升至 328k，电池占比下降、褐色占比上升。岸电桩数量由 1 增至 8 时，总成本由约 235k 降至 154k，同时褐色占比由 0.321 降至 0.036，说明岸电基础设施扩容对系统成本与绿色占比具有显著改善作用。紧迫度系数从 0.8 提升至 1.2 时，系统成本约下降 5\%，褐色占比同步下降，反映出更充裕的时间窗口能放大 SIMOPS 掩盖效应并提升绿色能源使用比例。
