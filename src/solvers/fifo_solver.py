...
def solve(instance: Instance, cfg: Dict[str, Any], logger) -> Dict[str, Any]:
    """Solve instance using an urgency-aware FIFO heuristic."""
    ...
    # Per-berth shore power usage tracking: shore_usage[k, t] = 1 if berth k occupied at time t
    ...
    order = sorted(
        ...
        key=lambda i: (
            arrival[i],
            deadlines[i] - arrival[i] - sp_dur[i],
        ),
    )
    ...
    for i in order:
        ...
        for t in range(start_min, horizon - duration + 1):
            if feasible_battery(t, duration):
                completion = max(arrival[i] + cargo[i], t + duration)
                tardy = max(0, completion - deadlines[i])
